...
export default function CarpinteriaPage() {
  ...
  const subir = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else setError(data.message || 'Error procesando DXF');
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Optimizador 2D (MaxRects)</h1>
      ...
      {resultado && (
        <>
          ...
          <canvas ref={canvasRef} width={1200} style={{ width: '100%', border: '1px solid #444' }} />
        </>
      )}
    </div>
  );
