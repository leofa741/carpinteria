...
// ✅ Aquí definimos que este componente recibe initialData e id
  ...
  const handleFileUpload = async (file: File, type: 'video' | 'thumbnail') => {
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        setMessage({ type: 'success', text: `✅ ${type === 'video' ? 'Video' : 'Imagen'} actualizado.` });
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-stone-100 dark:bg-stone-800/50 rounded-xl border border-dashed border-stone-300 dark:border-stone-700">
            <div>
              ...
              <p className="text-xs text-stone-500 mb-2 break-all truncate">Actual: {form.videoUrl}</p>
              <input type="file" accept="video/mp4,video/webm" onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], 'video')} disabled={uploading} className="block w-full text-sm text-stone-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-amber-100 file:text-amber-700 hover:file:bg-amber-200 dark:file:bg-amber-900/30 dark:file:text-amber-400 disabled:opacity-50" />
            </div>
            <div>
              <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Imagen de Portada</label>
              <p className="text-xs text-stone-500 mb-2 break-all truncate">Actual: {form.thumbnailUrl}</p>
              <input type="file" accept="image/*" onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], 'thumbnail')} disabled={uploading} className="block w-full text-sm text-stone-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-amber-100 file:text-amber-700 hover:file:bg-amber-200 dark:file:bg-amber-900/30 dark:file:text-amber-400 disabled:opacity-50" />
            </div>
          </div>
