...
export default function NuevaMaderaPage() {
  ...
  const handleFileUpload = async (file: File, type: 'video' | 'thumbnail') => {
    ...
    try {
      ...
      // ⚠️ IMPORTANTE: Asegúrate de que estas variables estén en tu .env.local
      ...
      // Opcional: asignar una carpeta en Cloudinary para mantener todo ordenado
      ...
      // Si Cloudinary devuelve un error, lo mostramos tal cual
      ...
      if (data.secure_url) {
        ...
        setMessage({ 
          type: 'success', 
          text: `✅ ${type === 'video' ? 'Video' : 'Imagen'} subido correctamente.` 
        });
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/maderas', {
        method: 'POST',
      ...
      setMessage({ type: 'success', text: '¡Madera y proceso guardados con éxito!' });
      
      // Redirigir después de 1.5 segundos
      setTimeout(() => {
        router.push('/maderas'); // O a /admin/maderas si tenés un listado
        router.refresh();
      }, 1500);
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          <h1 className="text-3xl font-bold text-stone-900 dark:text-stone-100 mb-2">
            Nuevo Registro de Madera
          </h1>
          <p className="text-stone-500 dark:text-stone-400 mb-8">
            Completá los datos del proceso artesanal y las especificaciones técnicas.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  required
                  placeholder="Ej: Ciprés de las Guaitecas"
                  className="w-full px-4 py-2 rounded-lg border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition"
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  name="tituloProceso"
                  value={form.tituloProceso}
                  onChange={handleChange}
                  required
                  placeholder="Ej: Secado Natural y Cepillado"
                  className="w-full px-4 py-2 rounded-lg border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                required
                rows={3}
                placeholder="Describí brevemente cómo se trabaja esta madera..."
                className="w-full px-4 py-2 rounded-lg border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition"
              />
            </div>

            {/* Sección 2: Archivos (Cloudinary) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-stone-100 dark:bg-stone-800/50 rounded-xl border border-dashed border-stone-300 dark:border-stone-700">
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Video del Proceso (MP4)</label>
                <input
                  type="file"
                  accept="video/mp4,video/webm"
                  onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], 'video')}
                  className="block w-full text-sm text-stone-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-amber-100 file:text-amber-700 hover:file:bg-amber-200 dark:file:bg-amber-900/30 dark:file:text-amber-400"
                />
                {form.videoUrl && <p className="text-xs text-green-600 mt-1">✓ Video cargado</p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Imagen de Portada (Thumbnail)</label>
                <input
                  type="file"
                  accept="image/*"
                  ...
                  className="block w-full text-sm text-stone-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-amber-100 file:text-amber-700 hover:file:bg-amber-200 dark:file:bg-amber-900/30 dark:file:text-amber-400"
                />
                {form.thumbnailUrl && <p className="text-xs text-green-600 mt-1">✓ Imagen cargada</p>}
              </div>
            </div>

            {/* Sección 3: Ficha Técnica */}
            <div className="border-t border-stone-200 dark:border-stone-800 pt-6">
              <h3 className="text-lg font-semibold text-stone-900 dark:text-stone-100 mb-4">Ficha Técnica (Para Arquitectos)</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="usoRecomendado"
                    value={form.usoRecomendado}
                    onChange={handleChange}
                    placeholder="Ej: Revestimientos int."
                    className="w-full px-3 py-2 rounded-lg border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-sm"
                  />
                </div>
                <div>
            ...
            <div className="flex items-center gap-3 pt-4">
              ...
              />
              <label htmlFor="destacado" className="text-sm font-medium text-stone-700 dark:text-stone-300">
                Marcar como "Destacado" (Aparecerá primero en la galería)
              </label>
            </div>
            ...
            <div className="pt-6">
              <button
                type="submit"
                disabled={loading}
                className="w-full md:w-auto px-8 py-3 bg-amber-600 hover:bg-amber-700 disabled:bg-amber-400 text-white font-semibold rounded-lg shadow-md transition-all duration-200 flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
