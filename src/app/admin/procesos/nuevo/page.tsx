...
export default function NuevoProcesoPage() {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    descripcionGeneral: '',
    clienteOtipo: '',
    pasos: [{ orden: 1, titulo: '', descripcion: '', mediaUrl: '', tipoMedia: 'video' }],
    destacado: false,
  });
 ...
 const handlePasoChange = (index: number, field: keyof Paso, value: string | number) => {
  // ✅ CORRECCIÓN: Usamos setForm con función de actualización para leer el estado más reciente
  setForm((prev) => {
  ...
  const removePaso = (index: number) => {
    if (form.pasos.length === 1) return; // Mantener al menos uno
   ...
   const handleFileUpload = async (file: File, pasoIndex: number, tipo: 'video' | 'imagen') => {
    ...
    try {
      ...
      if (data.secure_url) {
        // ✅ CORRECCIÓN CRÍTICA: Actualizamos mediaUrl y tipoMedia EN UN SOLO PASO
        setForm((prev) => {
          const nuevosPasos = [...prev.pasos];
        ...
        setMessage({ type: 'success', text: `✅ Archivo del paso ${pasoIndex + 1} subido correctamente.` });
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✅ VALIDACIÓN PREVENTIVA: Asegurarse de que ningún paso esté sin archivo
    const pasosSinArchivo = form.pasos.filter(p => !p.mediaUrl);
    if (pasosSinArchivo.length > 0) {
      setMessage({ 
        type: 'error', 
        text: `⚠️ Hay ${pasosSinArchivo.length} paso(s) sin archivo subido. Por favor, subí un video o imagen para cada paso.` 
      });
    ...
    try {
      // ... resto de tu código de envío a la API
      ...
      setMessage({ type: 'success', text: '¡Proceso guardado con éxito!' });
      setTimeout(() => {
    ...
    <main className="min-h-screen bg-stone-50 dark:bg-stone-950 px-4 py-12">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-stone-900 rounded-2xl shadow-xl border border-stone-200 dark:border-stone-800 p-8">
          <h1 className="text-3xl font-bold text-stone-900 dark:text-stone-100 mb-2">Nuevo Story Time</h1>
          <p className="text-stone-500 dark:text-stone-400 mb-8">Documentá el paso a paso de un proyecto artesanal.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" name="tituloProyecto" value={form.tituloProyecto} onChange={handleChange} required placeholder="Ej: Mesa de Quebracho Colorado" className="w-full px-4 py-2 rounded-lg border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:ring-2 focus:ring-amber-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Cliente / Tipo</label>
                  <input type="text" name="clienteOtipo" value={form.clienteOtipo} onChange={handleChange} placeholder="Ej: Pedido particular" className="w-full px-4 py-2 rounded-lg border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:ring-2 focus:ring-amber-500 outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Descripción General</label>
                <textarea name="descripcionGeneral" value={form.descripcionGeneral} onChange={handleChange} required rows={3} placeholder="Resumen del desafío y el resultado final..." className="w-full px-4 py-2 rounded-lg border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:ring-2 focus:ring-amber-500 outline-none" />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button 
                  type="button" 
                  onClick={addPaso} 
                  className="text-sm bg-stone-200 dark:bg-stone-800 hover:bg-stone-300 dark:hover:bg-stone-700 text-stone-800 dark:text-stone-200 px-3 py-1.5 rounded-lg font-medium transition-colors flex items-center gap-1"
                >
                  + Agregar Paso
                </button>
              </div>
              ...
              {form.pasos.map((paso, index) => (
                // ✅ AQUÍ ESTÁ EL CAMBIO: className condicional para resaltar en rojo si falta el archivo
                <div 
                  key={index} 
                  className={`p-5 rounded-xl space-y-4 relative group transition-all duration-300 ${
                    !paso.mediaUrl 
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input 
                      type="text" 
                      placeholder="Título del paso (Ej: Secado)" 
                      value={paso.titulo} 
                    ...
                    />
                    <select 
                      value={paso.tipoMedia} 
                      onChange={(e) => handlePasoChange(index, 'tipoMedia', e.target.value)} 
                      className="w-full px-3 py-2 rounded-lg border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-sm"
                    >
                  ...
                  <textarea 
                    placeholder="Descripción breve de lo que sucede en este paso..." 
                  ...
                  <div className="flex items-center gap-4">
                    <input 
                      type="file" 
                      accept={paso.tipoMedia === 'video' ? 'video/mp4,video/webm' : 'image/*'} 
                      onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], index, paso.tipoMedia)} 
                      disabled={uploading} 
                      className="block w-full text-xs text-stone-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-amber-100 file:text-amber-700 hover:file:bg-amber-200 dark:file:bg-amber-900/30 dark:file:text-amber-400" 
                    />
                    {activeStepUpload === index && <span className="text-xs text-amber-600 animate-pulse font-medium">Subiendo...</span>}
                  </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-6 border-t border-stone-200 dark:border-stone-800">
              ...
              <button type="submit" disabled={loading || uploading} className="w-full md:w-auto px-8 py-3 bg-amber-600 hover:bg-amber-700 disabled:bg-amber-400 text-white font-semibold rounded-lg shadow-md transition-all duration-200 flex items-center justify-center gap-2">
                {loading ? 'Guardando...' : 'Guardar Story Time'}
              </button>
            </div>
          </form>
        </motion.div>
